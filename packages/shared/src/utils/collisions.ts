...
export const pointCircleCollision = (point: Point, circle: Circle) =>
  dist(point, circle) <= circle.radius;
...
export const circleRectCollision = (circle: Circle, rect: Rectangle) => {
  ...
  const cx = Math.abs(circle.x - minX - halfWidth);
  const cy = Math.abs(circle.y - minY - halfHeight);
...
export const lineCircleCollision = (line: Line, circle: Circle) => {
  ...
  const lineDiff = Vec2.sub(line.start, line.end);
  ...
  const nearest = Vec2.add(line.start, projection);

  //  len2 of p
  const pLen2 = projection.x * projection.x + projection.y * projection.y;
